...
export const DepositModal = ({ depositAmount, SetDepositAmount, depositModalOpen, setDepositModalOpen, lockAmount,
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      const res = await walletService.deposit({ address: wallet.publicKey.toString(), amount: depositAmount, tx: tx })
      setPaying(false)
    ...
    <>
      {depositModalOpen ?
        <>
          ...
          <div className='absolute backdrop-blur-md w-[100vw] h-[100vh] z-40' onClick={() => setDepositModalOpen(false)} />
          {paying ?
            <div className='absolute backdrop-blur-md w-[100vw] h-[100vh] z-[50] flex justify-center items-center' >
              <DNA
                visible={true}
                height="80"
                width="80"
